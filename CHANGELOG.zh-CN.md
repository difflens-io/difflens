...
- 调整 CLI 和 skill 默认行为：除非显式传入 `--ignored-path`，否则默认不忽略任何路径。
...
- Web 页面继续保留常见易变字段的默认忽略项。
- 修复忽略路径输入框体验，用户可以在末尾追加逗号后继续自然输入下一个路径。

## 2026-07-26

### JSONL 格式化

- 点击“格式化输入”时，JSONL 会把每条记录按 JSON 方式格式化为多行对象块。
- 支持重新识别格式化后的连续 JSON 值，格式化后仍可按 JSONL 做结构化 diff。
- 输入区支持折叠格式化后的 JSONL 记录和嵌套 JSON 对象。
- 输入区差异显示中，格式化后的 JSONL 仍支持忽略路径遮罩。

### 验证

- 测试扩展到 30 个用例并全部通过，覆盖格式化 JSONL 解析、忽略路径和折叠。

### 工作区聚焦

- 将常驻显示的选项、统计和开源信息 band 改为紧凑概览条加单面板抽屉。
- 常驻保留格式和总差异数，详细选项、统计和 GitHub 信任信息通过切换按钮展开。
- “上一个 / 下一个”和差异项选择会同步滚动输入区到对应差异行。
- 默认开启“输入区显示差异”。
- 将“上一个 / 下一个”改为固定浮动控件，页面滚动后仍可继续点击。
- 增加基于路径的输入区定位，覆盖 JSON、JSONL、HTTP Request 请求体、cURL、Cookie、Properties、TOML、YAML 和文本行 diff。
- 输入区会突出当前选中的差异项，跳转前会自动展开包含目标位置的折叠区域。
- 修复差异项列表高度，长列表会在侧栏内部滚动，不再留下不可用的大块空白。
- 差异项侧栏的收起和展开按钮固定在同一个顶部位置，并让浮动差异导航避开侧栏按钮。
- 将浮动差异导航调整到差异项侧栏右侧附近，避免遮挡输入区内容。
- 修复 GitHub 查看源码和 Star 操作，点击时会明确打开仓库，避免穿透触发下方页面控件。
- 移除 GitHub 操作的当前页跳转兜底，打开 GitHub 时不会离开当前 DiffLens 页面。
